  ...
  <div>
    <v-container fluid style="padding:0">
      ...
      <v-col sm="7" style="padding-bottom: 0">
        ...
        <v-col sm="5" class="pl-16 pt-10 pr-16" style="background-color: #F7F3EE;">
          ...
          <p>
            The {{ product.name }} coffee maker combines precision brewing, top-quality construction, and a design that will elevate any environment.
            Designed for perfect brewing from 16 to 40 ounces, the Eight uses a smart brewing algorithm to mimic hand-made pour over coffee.
          </p>
          <v-row class="d-flex justify-center">
            <div style="width: 320px;">
              <v-btn class="mt-10" outlined :block="true"               @click="addProductToCart(product)">Add to cart</v-btn>
            </div>
          </v-row>
        </v-col>
      </v-row>
        ...
        <v-col sm="12"             v-for="item in reviews"             :key="item._id">
          ...
          >
            <v-card-title >
              ...
              <small class="grey--text">{{new Date(item.commentedAt).toLocaleString()}}</small>
              <v-spacer />
              <v-rating
                  :readonly="true"
                  :model-value="parseInt(item.rating)"
                  :length="5"
                  small
                  color="secondary"
              ></v-rating>
    ...
    <v-container  class="mt-10">
      ...
      <div class="d-flex justify-space-between align-center" >
        <div  v-for="relatedProduct in relatedProducts" :key="relatedProduct.slug" class="d-flex align-center  justify-center" style="flex-direction: column;cursor: pointer" @click="goToProduct(relatedProduct.slug)">
          <v-img
              height="200"
              width="200"
...
export default {
  ...
  computed: {
    averageReview(){
      const reviews = this.$store.state.product.reviews;
      if (reviews.length === 0) {
        return 0;
      }
      let totalReview = 0;
    ...
    reviewCount(){
      return this.$store.state.product.reviews.length;
    },
    products(){
  ...
  methods: {
    ...
    submitForm(){
      if (this.commentedText === ""){
        this.$store.dispatch("notification/showErrorMessage","Review text is missing")
        return
      }
      ...
      this.commentedText = ""
      this.rating = 5
      this.$store.dispatch("notification/showSuccessMessage","Review added successfully")
    },
    goToProduct(slug){
